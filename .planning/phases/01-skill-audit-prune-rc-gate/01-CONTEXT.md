...
**Target math (revised 2026-04-13 — v1.2 is cruft-only, see Corrections note at bottom of file):** 107 = 64 protected + 2 cruft removals (D-08, D-09) + 41 non-cruft candidates that will all stay in v1.2. **v1.2 Phase 1 ships a cruft-only prune: 107 − 2 cruft = 105 final.** The original training-duplicate rubric (D-10 through D-18) was attempted during Phase 1 execution and DEFERRED to v1.3 after the calibration pre-flight surfaced that clause (c) cannot distinguish training-duplicate skills from catalog cross-links in the current codebase. Target band for v1.2 Phase 1: **exactly 105**. The aggressive training-duplicate prune is deferred to v1.3 pending rubric redesign; see `.planning/research/v1.3-seeds/` for the partial audit artifacts that feed v1.3 planning.
...
> **[DEFERRED to v1.3 — 2026-04-13]** The entire rubric protocol (D-10 through D-18 across the next two subsections) was attempted during Phase 1 execution and deferred after the 5-skill calibration pre-flight surfaced that clause (c) cannot distinguish training-duplicate skills from catalog cross-links in the current codebase. All 4 "expected KEEP" and "expected PRUNE" calibration skills turned out to have structurally identical referrer patterns (bare pointers + catalog entries, no semantic dependencies). See the Corrections section at the bottom of this file for the evidence and `.planning/research/v1.3-seeds/` for the partial audit artifacts. **The decisions in D-10 through D-18 are preserved below as v1.3 research starting points, not as v1.2 execution state.** v1.2 Phase 1 ships cruft-only via D-08/D-09 using the removal/RC-gate machinery in D-19 through D-28.

- **D-10:** *(DEFERRED to v1.3)* The audit runs in two sequential passes, not as a single four-clause evaluation. This matters for simpler subagent prompting, separable failure-mode debugging, and clean user-facing PRUNE-LOG categorization.
- **D-11:** *(DEFERRED to v1.3)* **Pass 1 — Cruft filter (positive test):** For each candidate, ask *"Does this skill encode workflow, opinion, integration, or knowledge that is meaningfully tied to donnyclaude's supported tools (Claude Code, MCP servers, GSD engine, the rule files for the 13 supported languages, or the agents/hooks shipped in the distribution)?"* YES on any clause = passes the cruft filter and advances to Pass 2. NO on all clauses = cruft removal, logged in the "Cruft removals" section of PRUNE-LOG.md with category `origin-drift` or `unrelated-project-installer` or `deprecated-upstream-tool` as appropriate.
- **D-12:** *(DEFERRED to v1.3)* **Pass 2 — Training-duplicate rubric (all three must be TRUE to prune):**
...
- **D-13:** *(DEFERRED to v1.3)* **Clause (c) evaluation order-dependence gotcha:** Clause (c) MUST be evaluated against the **original 107-skill repo state**, not the in-progress pruned state. If the audit subagent processes skills alphabetically and flags `plankton-code-quality` for removal before reaching `verification-loop`, the reference from plankton → verification-loop disappears from the referrer set, and verification-loop's clause-(c) evidence changes mid-audit. The subagent must snapshot the reference graph BEFORE any prune decisions are applied, then evaluate all candidates against the frozen snapshot. This is a concrete prompt requirement for the audit subagent — not a nice-to-have.
- **D-14:** *(INVALIDATED by calibration evidence, 2026-04-13 — preserved for v1.3 context)* **Original text:** "Already-verified clause-(c) keeps (from context-writing spot-checks): `tdd-workflow` has 7 references... `e2e-testing` has 2 references..." **What actually happened during execution:** grep against the frozen snapshot found 9 non-cruft referrers for tdd-workflow (not 7) and 2 for e2e-testing (matching). BUT inspection of every referrer showed they are all bare `see skill: X` pointers or catalog entries — structurally identical to python-patterns' 5 and golang-patterns' 6 non-cruft referrers, which D-16 expected to PRUNE. The one referrer that looked like a semantic dependency (`strategic-compact/SKILL.md:106` for tdd-workflow) turned out to be illustrative documentation in a "Trigger-Table Lazy Loading" section, with no corresponding logic in `strategic-compact/suggest-compact.sh`. **Therefore tdd-workflow and e2e-testing are NOT "confirmed KEEPs via clause (c) protection" — they are KEEPs only because the rubric as written treats bare pointers as protection.** v1.3 rubric redesign must decide whether bare pointers count as protection. See `.planning/research/v1.3-seeds/PRUNE-VERDICT-partial-v1.json` for the per-referrer type classification.

### Rubric application — hybrid human-subagent with calibration

- **D-15:** *(DEFERRED to v1.3)* The audit is executed by a subagent that reads each candidate SKILL.md, evaluates the two passes, and emits structured `PRUNE-VERDICT.json`. The human (Donovan) reviews every PRUNE and every UNCERTAIN verdict; KEEP verdicts are skimmed but not individually audited. Rationale: false-positive prunes are the asymmetric-risk direction (user-facing, harder to reverse even with `git mv`); false-negative keeps are recoverable in v1.3 with no user impact. **Note:** the asymmetric-risk rationale held under stress — the calibration gate firing protected v1.2 from shipping 41 incorrectly-verdicted skills. This design principle should carry forward to v1.3.
- **D-16:** *(DEFERRED to v1.3 — expected verdicts for python-patterns, golang-patterns, and eval-harness turned out to be incorrect; see Corrections)* **5-skill calibration pre-flight (mandatory before full 41-candidate pass):** The audit subagent first processes a pinned held-out set of 5 pre-judged skills. Human compares verdicts to expectations. **5/5 agreement = proceed to full pass. 4/5 or worse = tighten the rubric prompt, then re-run calibration before the full pass.** The calibration skills are:
...
### 2026-04-13 — candidate count 43→41

The original context (and downstream plans 01-01 and 01-02) described "43 rubric candidates." The correct number is **41**. Derivation: `ls packages/skills` returns 107 directories; `grep -v '^gsd-' | grep -vE '^(skill-stocktake|humanizer|strategic-compact|frontend-slides|configure-ecc)$'` returns 42 (excludes 60 gsd-* + 4 non-gsd protected + configure-ecc); then D-09 continuous-learning resolution drops the loser → **41 rubric candidates**. The original "Resulting count: 43" in 01-01-PLAN.md omitted the continuous-learning subtraction. **Cascade:** derived counts updated in D-07 ("other 37 candidates", was 39), D-16 ("full 41-candidate pass", was 43), D-17 ("36 unseen candidates", was 38), 01-01-PLAN.md acceptance criteria (candidate count now 41), and 01-02-PLAN.md line 42 narrative.

### 2026-04-13 — rubric deferred to v1.3, v1.2 ships cruft-only (the big one)

**The 5-skill calibration pre-flight fired and correctly detected that the rubric's clause (c) cannot distinguish training-duplicate skills from catalog cross-links in this codebase.** The calibration was designed to test whether the subagent could reproduce 5 pre-judged verdicts (2 expected PRUNE, 2 expected KEEP, 1 borderline). When the rubric was applied honestly against the frozen reference snapshot:

| Skill             | Expected | Actual      | Non-cruft referrers | Referrer types                                |
|-------------------|----------|-------------|---------------------|-----------------------------------------------|
| python-patterns   | PRUNE    | KEEP        | 5                   | 3 bare-pointers, 1 catalog, 1 example-mention |
| golang-patterns   | PRUNE    | KEEP        | 6                   | 4 bare-pointers, 2 catalog                    |
| tdd-workflow      | KEEP     | KEEP ✓      | 9                   | 4 bare-pointers, 5 catalog                    |
| e2e-testing       | KEEP     | KEEP ✓      | 2                   | 2 bare-pointers                               |
| eval-harness      | PRUNE    | UNCERTAIN   | 0                   | (1 referrer, cruft-only from configure-ecc)   |

**agreement_count: 2/5. proceed: false. Calibration gate blocked the full 41-candidate pass.**

The root cause surfaced when we inspected the referrers by type: **all 5 calibration skills have structurally identical referrer patterns** — bare `see skill: X` pointers in reviewer agents, catalog entries in rule files and commands, no semantic invocations. The referrer in `strategic-compact/SKILL.md:106` that looked like it might be a semantic dependency turned out to be illustrative documentation in a "Trigger-Table Lazy Loading" section; two of the three example skills in that table (`security-review`, `deployment-patterns`) don't exist, and the `suggest-compact.sh` hook script contains zero references to any of the skill names.

**Implication:** The D-14 claim that "`tdd-workflow` has 7 references — confirmed KEEP via clause (c) protection" is technically true but strategically misleading. tdd-workflow is KEPT by the rubric only because the rubric counts bare pointers as protection. Under any refinement of clause (c) that excludes bare pointers (to enable the training-duplicate prune the rubric was designed for), tdd-workflow and e2e-testing would ALSO prune. There is no pattern in the current codebase that distinguishes the "expected KEEP" calibration skills from the "expected PRUNE" ones.

**This invalidates the entire training-duplicate rubric as a mechanism for v1.2.** Options considered:

1. Rescope target band upward from [75, 85] to [98, 105], run the full audit anyway, accept 3-5 prunes as the output. Rejected: keeps a broken rubric in-tree, uses 41-candidate machinery to deliver <5 verdicts, and does not capture the calibration-gate lesson.
2. Tighten clause (c) to exclude bare pointers, reopen D-14, re-author audit prompt, re-run calibration. Rejected: the evidence shows there is no "semantic dependency" pattern in the codebase to carve the new rubric around — the refinement has no signal to latch onto.
3. **Cruft-only v1.2 + defer rubric to v1.3 (selected).** Ship the 2 unambiguous cruft removals (configure-ecc per D-08, continuous-learning loser per D-09). Target band for v1.2 Phase 1: exactly 105 (107 − 2 cruft). Preserve the audit work as v1.3 research seeds. Move rubric redesign to v1.3 milestone.
4. Abort Phase 1 and redo planning from scratch. Rejected: the 28 decisions in this file are mostly sound; only D-10 through D-18 and the target math are affected. A rewrite would reproduce the same artifacts minus 3-4 broken decisions. An amendment is the right shape of response.

**Chosen: Option 3.** v1.2 Phase 1 now ships cruft-only. Status changes:
- **D-10 through D-18:** DEFERRED to v1.3 (preserved inline as research starting points, marked in place).
- **D-14 specifically:** INVALIDATED with detailed note. The "7 referrers = clause-c protection" claim is factually true but strategically misleading.
- **Target band:** [75, 85] → exactly 105.
- **Plan restructure:** 5 plans → 3. Old 01-01 (audit subagent) and 01-02 (human review) deleted. New 01-01 = cruft-only execution (adapted from old 01-03). Old 01-04 → 01-02. Old 01-05 → 01-03.
- **Research seeds:** `audit-subagent-prompt-v1.md` + `PRUNE-VERDICT-partial-v1.json` + `reference-graph-snapshot-5skill-sample.txt` + README moved to `.planning/research/v1.3-seeds/`.
- **v1.3 backlog:** Rubric redesign research — "what does 'semantic dependency' mean in a catalog-linked distribution?" Added to PROJECT.md backlog.
- **Release notes tone:** v1.2.0-rc.1 release notes describe the cruft removals and explicitly state that the broader training-duplicate prune is deferred to v1.3 pending rubric redesign.
- **D-19 through D-28 (RC gate, removal trail, CHANGELOG):** STAY. They apply to the 2 cruft removals.

**Durable lesson captured in PROJECT.md Key Decisions:** The calibration gate worked as designed. Adding the 5-skill pre-flight during context-gathering was the specific move that prevented v1.2 from shipping 41 incorrectly-verdicted skills. The asymmetric-risk framing in D-15 held under stress. The gate-before-full-pass protocol is validated for any future audit-subagent work.

---

*Phase: 01-skill-audit-prune-rc-gate*
*Context gathered: 2026-04-12*
*Corrections landed: 2026-04-13 (candidate count fix + rubric deferral to v1.3)*
