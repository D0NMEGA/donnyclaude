...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  for (const planFile of planFiles) {
    ...
    const _wave = parseInt(fm.wave, 10);
    const wave = Number.isNaN(_wave) ? 1 : _wave;
...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    const phaseEntry = `\n### Phase ${_newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /donny-plan-phase ${_newPhaseId} to break down)\n`;
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    // Build phase entry
    const phaseEntry = `\n### Phase ${_decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /donny-plan-phase ${_decimalPhase} to break down)\n`;
...
// Mark a phase intentionally deferred without claiming it is done. Writes a
// *-DEFERRED.md marker the autonomous loop skips (via roadmap analyze's `actionable`
// field), so a skipped/gappy phase stops re-prompting WITHOUT being recorded complete
// or having its requirements marked satisfied (which `phase complete` would do).
function cmdPhaseDefer(cwd, phaseNum, options, raw) {
  if (!phaseNum) {
    error('phase number required for phase defer');
  }
  const phaseInfo = findPhaseInternal(cwd, phaseNum);
  if (!phaseInfo || !phaseInfo.directory) {
    output({ deferred: false, error: `Phase ${phaseNum} has no directory to defer`, phase: phaseNum }, raw, 'no_directory');
    return;
  }
  const today = new Date().toISOString().split('T')[0];
  const reason = (options && options.reason) ? String(options.reason) : 'deferred by user';
  const dirBase = path.basename(phaseInfo.directory);
  const prefix = (dirBase.match(/^(\d+(?:\.\d+)?)/) || [])[1] || normalizePhaseName(phaseNum);
  const markerPath = path.join(cwd, phaseInfo.directory, `${prefix}-DEFERRED.md`);
  const body = `---\nphase: ${phaseInfo.phase_number}\ndeferred: ${today}\n---\n\n# Phase ${phaseInfo.phase_number} - Deferred\n\n**Reason:** ${reason}\n\nThis phase was intentionally left incomplete. It is NOT counted as complete and its\nrequirements are NOT marked satisfied. The autonomous loop skips deferred phases instead\nof re-prompting them. Delete this file (or re-plan the phase) to pick it back up.\n`;
  fs.writeFileSync(markerPath, body, 'utf-8');
  output({ deferred: true, phase: phaseInfo.phase_number, marker: path.relative(cwd, markerPath), reason }, raw, 'deferred');
}
