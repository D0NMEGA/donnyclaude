...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
  ...
  process.stderr.write(
    `[donny-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { donny_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when Donny hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .donny/WAITING.json if .donny exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const donnyDir = fs.existsSync(path.join(cwd, '.donny')) ? path.join(cwd, '.donny') : planningDir(cwd);
  const waitingPath = path.join(donnyDir, 'WAITING.json');
  ...
  try {
    fs.mkdirSync(donnyDir, { recursive: true });
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.donny', 'WAITING.json'),
