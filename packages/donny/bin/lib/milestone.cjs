...
// Deterministic milestone-completion check: ready to archive only when every phase that
// has plans has a summary for each (progress 100%, mirroring roadmap analyze). complete-
// milestone gates its irreversible archive+tag on this instead of an LLM glance.
function milestoneReadiness(phases) {
  const incomplete = phases
    .filter(p => (p.planCount || 0) > 0 && (p.summaryCount || 0) < p.planCount)
    .map(p => p.id);
  const totalPlans = phases.reduce((s, p) => s + (p.planCount || 0), 0);
  const totalSummaries = phases.reduce((s, p) => s + (p.summaryCount || 0), 0);
  const progress_percent = totalPlans > 0 ? Math.min(100, Math.round((totalSummaries / totalPlans) * 100)) : 0;
  return { ready: totalPlans > 0 && incomplete.length === 0, progress_percent, incomplete };
}
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      phaseStats.push({ id: dir, planCount: plans.length, summaryCount: summaries.length });
  ...
  // Deterministic completion gate before the irreversible archive + git tag.
  // Refuse to "complete" a milestone whose phases are not all summarized,
  // unless the caller explicitly forces it.
  const readiness = milestoneReadiness(phaseStats);
  if (!readiness.ready && !options.force) {
    output({
      error: 'Milestone not complete',
      ready: false,
      progress_percent: readiness.progress_percent,
      incomplete: readiness.incomplete,
      hint: 'finish/verify the incomplete phases, or pass --force to archive anyway',
    }, raw);
    return;
  }

  // Past the gate: create the archive directory and perform the irreversible writes.
