  ...
  if (items.length === 0 && blockLines.length > 0) {
    ...
    if (nonEmptyLines > 0) {
      process.stderr.write(
        `[donny-tools] WARNING: must_haves.${blockName} block has ${nonEmptyLines} content lines but parsed 0 items. ` +
...
const FRONTMATTER_SCHEMAS = {
  // requirements is REQUIRED and must be non-empty: a plan that maps to zero
  // ROADMAP requirement IDs breaks the goal-backward traceability the verifier
  // depends on, so it is enforced here rather than by a skippable shell check.
  plan: { required: ['phase', 'plan', 'type', 'wave', 'depends_on', 'files_modified', 'autonomous', 'requirements', 'must_haves'], nonEmpty: ['requirements'] },
  summary: { required: ['phase', 'plan', 'subsystem', 'tags', 'duration', 'completed'] },
  verification: { required: ['phase', 'verified', 'status', 'score'] },
  // A UI-SPEC drives frontend implementation; a malformed or near-empty one must not
  // pass silently. Identity/lifecycle fields are nonEmpty; shadcn_initialized/preset are
  // required-only (their parsed values false/"none" are legitimately non-empty).
  'ui-spec': { required: ['phase', 'slug', 'status', 'shadcn_initialized', 'preset', 'created'], nonEmpty: ['phase', 'slug', 'status'] },
};

// A value is "empty" if it carries none of the information a required field
// promises: null/undefined, blank string, empty array, or empty object. Real
// scalars (0, false) are not empty.
...
// Pure schema check, split from I/O so it is unit-testable. `missing` lists
// required keys absent entirely; `empty` lists nonEmpty keys present but blank.
function validateAgainstSchema(fm, schema) {
  const missing = schema.required.filter(f => fm[f] === undefined);
  const present = schema.required.filter(f => fm[f] !== undefined);
  const empty = (schema.nonEmpty || []).filter(f => fm[f] !== undefined && isEmptyValue(fm[f]));
  return { valid: missing.length === 0 && empty.length === 0, missing, empty, present };
}
...
function cmdFrontmatterValidate(cwd, filePath, schemaName, raw) {
  ...
  if (!content) { output({ error: 'File not found', path: filePath }, raw); return; }
  const fm = extractFrontmatter(content);
  const { valid, missing, empty, present } = validateAgainstSchema(fm, schema);
  output({ valid, missing, empty, present, schema: schemaName }, raw, valid ? 'valid' : 'invalid');
}
