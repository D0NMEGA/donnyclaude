...
/**
 * Remove stale donny-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'donny-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'donny-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `donny-${Date.now()}.json`);
...
function loadConfig(cwd) {
  ...
  const defaults = {
    ...
    max_replan_iterations: 2, // bound the plan<->check revision loop; in --auto, escalate to a decision checkpoint at the cap
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      process.stderr.write(
        `donny-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
      );
    }
 ...
 * Applied at write points so Donny-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 *
 * Resolution order:
 * 1. If DONNY_PROJECT is set (env var or explicit `project` arg), routes to
 ...
 * 2. If DONNY_WORKSTREAM is set, routes to `.planning/workstreams/{ws}/`.
 * 3. Otherwise returns `.planning/`.
 *
 * DONNY_PROJECT and DONNY_WORKSTREAM can be combined:
 *   `.planning/{project}/workstreams/{ws}/`
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks DONNY_WORKSTREAM env var
 * @param {string} [project] - explicit project name; if omitted, checks DONNY_PROJECT env var
 */
function planningDir(cwd, ws, project) {
  if (project === undefined) project = process.env.DONNY_PROJECT || null;
  if (ws === undefined) ws = process.env.DONNY_WORKSTREAM || null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`DONNY_PROJECT contains invalid path characters: ${project}`);
  }
  if (ws && BAD_SEGMENT.test(ws)) {
    throw new Error(`DONNY_WORKSTREAM contains invalid path characters: ${ws}`);
  }
 ...
 *
 * Order:
 * 1. Explicit runtime/session env vars (`DONNY_SESSION_KEY`, `CODEX_THREAD_ID`, etc.)
...
function getSessionScopedWorkstreamFile(cwd) {
  ...
  const dirPath = path.join(os.tmpdir(), 'donny-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
...
/**
 * Resolve the agents directory from the Donny install location.
 * donny-tools.cjs lives at <configDir>/donny/bin/donny-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * DONNY_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with donny-tools.cjs.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  if (process.env.DONNY_AGENTS_DIR) {
    return process.env.DONNY_AGENTS_DIR;
  }
  // __dirname is donny/bin/lib/ → go up 3 levels to configDir
 ...
 *
 * Recognises both standard format (donny-planner.md) and Copilot format
 * (donny-planner.agent.md). Copilot renames agent files during install (#1512).
 *
