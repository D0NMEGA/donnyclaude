...
// The autonomous loop's phase-selection predicate, made deterministic. A phase is
// actionable when it is not yet done (incomplete on disk OR its ROADMAP box is unchecked
// - the original filter) AND it has not been explicitly deferred. Deferral breaks the
// re-prompt loop without marking the phase complete or satisfying its requirements.
function phaseIsActionable(phase) {
  if (!phase || phase.deferred === true) return false;
  return phase.disk_status !== 'complete' || phase.roadmap_complete !== true;
}
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    try {
      ...
      if (dirMatch) {
        ...
        deferred = phaseFiles.some(f => /-DEFERRED\.md$/i.test(f) || f === 'DEFERRED.md');
    ...
    // Phases completed before Donny tracking (or via external tools) may lack
    ...
    phases.push({
      ...
      deferred,
      actionable: phaseIsActionable({ disk_status: diskStatus, roadmap_complete: roadmapComplete, deferred }),
    });
  }
