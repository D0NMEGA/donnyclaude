 ...
 *
 * This module centralizes security checks for Donny tooling. Because Donny generates
...
const INJECTION_PATTERNS = [
  ...
  // Note: <instructions> is excluded — Donny uses it as legitimate prompt structure
...
//
// Donny generates markdown maps/docs from a codebase and commits them. A doc that
// quoted a real API key would leak it into git history. These patterns are versioned
// and tested here (not retyped inline in a workflow) so the map-codebase commit gate
// is deterministic and the patterns evolve in one place.

const SECRET_PATTERNS = [
  { kind: 'openai', re: /sk-[a-zA-Z0-9]{20,}/ },
  { kind: 'stripe', re: /sk_(?:live|test)_[a-zA-Z0-9]{10,}/ },
  { kind: 'github-token', re: /gh[posu]_[A-Za-z0-9]{36,}/ },
  { kind: 'gitlab-token', re: /glpat-[A-Za-z0-9_-]{20,}/ },
  { kind: 'aws-access-key', re: /AKIA[A-Z0-9]{16}/ },
  { kind: 'slack-token', re: /xox[baprs]-[A-Za-z0-9-]{10,}/ },
  { kind: 'private-key', re: /-----BEGIN[A-Z ]*PRIVATE KEY-----/ },
  { kind: 'jwt', re: /eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/ },
];

// Mask a matched secret so a finding never reproduces the credential it flagged.
function redactSecret(s) {
  return (String(s).slice(0, 4) + '***').slice(0, 12);
}

/**
 * Scan text for leaked credentials. Returns one finding per (line, pattern) match,
 * each carrying a 1-based line number and a REDACTED preview - never the full secret.
 * @param {string} text
 * @returns {Array<{ kind: string, line: number, preview: string }>}
 */
function scanForSecrets(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    for (const { kind, re } of SECRET_PATTERNS) {
      ...
      if (m) findings.push({ kind, line: i + 1, preview: redactSecret(m[0]) });
    }
  }
  return findings;
}
...
/**
 * CLI: scan the given files for leaked secrets. Emits { clean, files_scanned, missing,
 * findings:[{ file, kind, line, preview }] } and sets a non-zero exit code when not
 * clean, so both `... && commit` chaining and JSON `clean` inspection gate the commit.
 */
function cmdScanSecrets(cwd, fileArgs, raw) {
  const targets = (fileArgs || []).filter(a => a && !a.startsWith('--'));
  const findings = [];
  const scanned = [];
  const missing = [];
  for (const t of targets) {
    const abs = path.isAbsolute(t) ? t : path.join(cwd, t);
    if (!(fs.existsSync(abs) && fs.statSync(abs).isFile())) { missing.push(t); continue; }
    scanned.push(t);
    for (const f of scanForSecrets(safeReadFile(abs) || '')) findings.push({ file: t, ...f });
  }
  const clean = findings.length === 0;
  output({ clean, files_scanned: scanned.length, missing, findings }, raw, clean ? 'clean' : 'secrets-found');
