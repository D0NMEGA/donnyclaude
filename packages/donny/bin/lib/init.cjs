...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'donny-executor'),
    verifier_model: resolveModelInternal(cwd, 'donny-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'donny-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'donny-planner'),
    checker_model: resolveModelInternal(cwd, 'donny-plan-checker'),

    // Workflow flags
    research_enabled: config.research,
    browser_research_enabled: config.browser_research,
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'donny-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'donny-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'donny-roadmapper'),
...
function cmdInitVerifyWork(cwd, phase, raw) {
  ...
  // The phase's assigned REQ-IDs from the ROADMAP, so verify-work can flag a
  // requirement that was dropped during execution and so appears in no SUMMARY.
  const reqMatch = roadmapPhase?.section?.match(/^\*\*Requirements\*\*:[^\S\n]*([^\n]*)$/m);
  const reqExtracted = reqMatch
    ? reqMatch[1].replace(/[\[\]]/g, '').split(',').map(s => s.trim()).filter(Boolean).join(', ')
    : null;
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'donny-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /donny-init first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /donny-init first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/donny-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/donny-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/donny-discuss-phase ${phase.number}`,
      });
    }
  }
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`donny-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'donny-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'donny-executor', 'donny-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills donny-executor 2>/dev/null)
 */
