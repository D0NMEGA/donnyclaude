...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /donny-init to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- Donny:${sectionName}-start`;
  const endMarker = `<!-- Donny:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- Donny:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- Donny:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip Donny's own installed skills — only surface project-specific skills
      if (entry.name.startsWith('donny-')) continue;
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'donny', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/donny-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'donny', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/donny-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by Donny from ${dataSource}. Run \`/donny-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- Donny:profile-start -->';
    const endMarker = '<!-- Donny:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## Donny Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- Donny:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- Donny:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- Donny:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- Donny:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /donny-profile-user to unlock Developer Profile.';
