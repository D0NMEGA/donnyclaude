...
The `--ws <name>` flag scopes Donny operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `DONNY_WORKSTREAM` environment variable (per-instance)
...
Donny now prefers a session-scoped pointer keyed by runtime/session identity
(`DONNY_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When Donny resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `DONNY_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, Donny does not keep probing. It falls
...
non-interactive, Donny intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, Donny also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, Donny
...
Donny does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${DONNY_WS}` which:
...
```bash
# All donny-tools commands accept --ws
node donny-tools.cjs state json --ws feature-a
node donny-tools.cjs find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
DONNY_SESSION_KEY=my-terminal-a node donny-tools.cjs workstream set feature-a
DONNY_SESSION_KEY=my-terminal-a node donny-tools.cjs state json
DONNY_SESSION_KEY=my-terminal-b node donny-tools.cjs workstream set feature-b
DONNY_SESSION_KEY=my-terminal-b node donny-tools.cjs state json

# Workstream CRUD
node donny-tools.cjs workstream create <name>
node donny-tools.cjs workstream list
node donny-tools.cjs workstream status <name>
node donny-tools.cjs workstream complete <name>
```
