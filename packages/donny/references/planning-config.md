...
| `git.phase_branch_template` | `"donny/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"donny/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/donny:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using donny-tools.cjs (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs: update state" --files .planning/STATE.md
...
**Note:** Most Donny operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Standard Donny behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `donny/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `donny/v1.0-mvp`)
