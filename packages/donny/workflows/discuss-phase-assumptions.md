...
- donny-discuss-researcher - Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-discuss-researcher 2>/dev/null)
```
...
Use /donny-progress to see available phases.
```
Exit workflow.
...
- When spawning the donny-discuss-researcher, pass the lens list so it can flag which lenses apply
...
Spawn a `donny-discuss-researcher` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="donny-discuss-researcher", prompt="""
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/donny-plan-phase ${PHASE}`

---

**Also available:**
- `/donny-plan-phase ${PHASE} --skip-research` - plan without research
- `/donny-ui-phase ${PHASE}` - generate UI design contract (if frontend work)
...
Launch: `Skill(skill="donny-plan-phase", args="${PHASE} --auto")`
