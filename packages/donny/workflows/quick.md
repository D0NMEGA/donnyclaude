<purpose>
Execute small, ad-hoc tasks with Donny guarantees (atomic commits, STATE.md tracking).

Quick triages the task first and picks the lightest path that fits - no flag needed:
- **Trivial one-liner** (typo, config value, missing import, version bump, forgotten commit): run INLINE in this context - no subagents, no PLAN.md - stage the touched files explicitly, commit, and log to STATE.md. This is the default for anything you could describe in one sentence and finish in under two minutes.
- **Real change:** spawn donny-planner (quick mode) + donny-executor(s), track the task in `.planning/quick/`, and update STATE.md's "Quick Tasks Completed" table.

Passing any quality flag (`--discuss`, `--research`, `--validate`, `--full`) forces the pipeline path.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► QUICK TASK (DISCUSS + RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Step 1.5: Triage - inline or pipeline**

Before initializing the pipeline, decide whether this task even needs one.

**Force the pipeline (skip to Step 2) if ANY quality flag is set:** `$FULL_MODE`, `$DISCUSS_MODE`, `$RESEARCH_MODE`, or `$VALIDATE_MODE`. Asking for discussion, research, or validation signals a real change.

**Otherwise assess `$DESCRIPTION`** (glance at the repo if the scope is unclear). The task is *trivial* when ALL of these hold:
- It can be described in one sentence.
- It touches at most ~3 files and takes under ~2 minutes.
- It needs no new dependencies and no architectural change.
- It needs no research to know how to do it.

Examples of trivial: fix a typo, change a config value, add a missing import, rename a local variable, bump a version number, add a `.gitignore` entry, commit forgotten work.

- **Trivial -> run Step 1.6 (inline) and STOP.** Do not init the pipeline, do not create a `.planning/quick/` directory, do not spawn any subagent. The inline path needs no ROADMAP.md - it works in any repo.
- **Not trivial -> continue to Step 2 (pipeline).**

If you start the inline path and discover mid-way that the task is larger than it looked (more than ~3 files, or it needs research), stop before committing, tell the user, and restart on the pipeline path from Step 2.

---

**Step 1.6: Inline execution (trivial path only)**

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► QUICK TASK (INLINE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Trivial task - running inline, no subagents: ${DESCRIPTION}
```

**1.6a. Do the work directly.** Read the relevant file(s), make the change(s), and sanity-check (run the obvious existing test, or a quick manual check). No PLAN.md, no SUMMARY.md, no Task - this is the whole point of the inline path.

**1.6b. Commit with explicit staging.** Stage only the files you actually edited in 1.6a - NEVER `git add -A`, which can sweep in `.env`, credentials, build output, or unrelated work. Collect the touched paths into `$CHANGED_FILES` and stage them by name:

```bash
# $CHANGED_FILES = the exact paths you edited (quote each one)
git add -- ${CHANGED_FILES}
git commit -m "${TYPE}: ${concise description}"   # TYPE: fix | feat | docs | chore | refactor
```

Use conventional-commit format. If nothing actually changed (e.g. the fix was already present), say so and stop - do not create an empty commit. Capture the hash:

```bash
commit_hash=$(git rev-parse --short HEAD)
```

**1.6c. Log to STATE.md (only if the table already exists).** If `.planning/STATE.md` exists and already contains a `Quick Tasks Completed` table, append one row that matches that table's existing columns using the **Edit tool** - not a raw `>>` redirect, which appends a mismatched-width row and corrupts the table. Inline tasks have no artifact directory, so put `-` in the Directory column (and `Inline` in the Status column if the table has one). Use today's date and the short commit hash; mark the `#` column `inline`:

```
| inline | ${DESCRIPTION} | ${date} | ${commit_hash} | - |
```

If STATE.md has no such table, skip this step silently - inline tasks do not create planning scaffolding.

**1.6d. Report and stop.**
```
✓ Done (inline): ${DESCRIPTION}
  Commit: ${commit_hash}
  Files:  ${CHANGED_FILES}
```
No next-step routing. The inline path ends here - do NOT continue to Step 2.
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-planner 2>/dev/null)
AGENT_SKILLS_EXECUTOR=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-executor 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-plan-checker 2>/dev/null)
AGENT_SKILLS_VERIFIER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-verifier 2>/dev/null)
```
...
**If `roadmap_exists` is false:** Error - Quick mode requires an active project with ROADMAP.md. Run `/donny-init` first.
...
Spawn donny-executor with plan reference:
...
Stage and commit quick task artifacts. This step MUST always run - even if the executor already committed some files (e.g. when running without worktree isolation). The `donny-tools commit` command handles already-committed files gracefully.
...
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
DONNY ► QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /donny-quick ${DONNY_WS}
```
...
<success_criteria>
- [ ] Task triaged: trivial + no quality flags -> inline path; otherwise pipeline path
- [ ] (inline) Runs in-context with no subagent and no PLAN.md, then stops after the commit
- [ ] (inline) Commit stages only the changed files by name - never `git add -A`
- [ ] (inline) STATE.md row appended via Edit matching the existing table columns (skipped if no table)
- [ ] (pipeline) ROADMAP.md validation passes
