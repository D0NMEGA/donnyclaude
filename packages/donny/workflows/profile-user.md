...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing donny-tools.cjs subcommands and the donny-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/donny/references/ui-brand.md (display patterns)
- @$HOME/.claude/donny/agents/donny-user-profiler.md (profiler agent definition)
- @$HOME/.claude/donny/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp "$HOME/.claude/donny/USER-PROFILE.md" "$HOME/.claude/donny/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- Stores the profile at $HOME/.claude/donny/USER-PROFILE.md (preserved across /donny-update, which reapplies it as a local patch)

Does not:
- Send anything to external services
- Include sensitive content (API keys and passwords are auto-excluded)
```
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /donny-profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/donny/bin/donny-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/donny/bin/donny-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn donny-user-profiler agent using Task tool:**

Use the Task tool to spawn the `donny-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/donny/references/user-profiling.md`
...
Reference: @$HOME/.claude/donny/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/donny/bin/donny-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/donny-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/donny/bin/donny-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/donny-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/donny/bin/donny-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "Profile written to $HOME/.claude/donny/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/donny-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/donny/USER-PROFILE.md" and jump to step 10.
...
**For /donny-dev-preferences (if selected):**

```bash
node $HOME/.claude/donny/bin/donny-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "Generated /donny-dev-preferences at $HOME/.claude/commands/donny/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/donny/bin/donny-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/donny/bin/donny-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any donny-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/donny/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► PROFILE COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/donny/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  /donny-dev-preferences   $HOME/.claude/commands/donny/dev-preferences.md
...
- [ ] Artifacts generated sequentially via donny-tools.cjs subcommands
