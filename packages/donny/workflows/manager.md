...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init manager)
...
- `manager_flags.discuss` - appended to `/donny-discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` - appended to plan agent init command
- `manager_flags.execute` - appended to execute agent init command

These are empty strings by default. Set via: `donny-tools config-set manager.flags.discuss "--auto --analyze"`

**Guard interactive-only modes:** the manager dispatches discuss inside an auto-refreshing loop, so blocking modes stall it. If `manager_flags.discuss` contains `--power` (opens a file-based Q&A that waits for a human and freezes the refresh loop) or `--chain` (re-triggers plan+execute, which the manager already dispatches itself), warn once at startup and ignore that token. For unattended discuss use `--auto` instead.
...
All {phase_count} phases done. Ready for final steps:
  -> /donny-verify-work - run acceptance testing
  -> /donny-complete-milestone - archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="donny-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="donny-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in Donny config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="donny-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Task(
  ...
  prompt="You are running the Donny plan-phase workflow for phase {N} of the project.
...
Skill(skill=\"donny-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the Donny execute-phase workflow for phase {N} of the project.
...
Skill(skill=\"donny-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill - use `Skill(skill="donny-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="donny-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill - use `Skill(skill="donny-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="donny-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /donny-manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/donny-manager` or `/donny-progress` invocation.
