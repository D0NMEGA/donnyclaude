<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /donny-plan-phase --gaps.
...
<template>
@$HOME/.claude/donny/templates/UAT.md
</template>
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init verify-work "${PHASE_ARG}")
...
Parse JSON for: `planner_model`, `checker_model`, `commit_docs`, `phase_found`, `phase_dir`, `phase_number`, `phase_name`, `phase_req_ids`, `has_verification`, `uat_path`.
</step>
...
Provide a phase number to start testing (e.g., /donny-verify-work 4)
```
...
```
UI_PHASE_FLAG=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
<step name="requirement_drift_check">
**Surface requirements that were dropped (no SUMMARY covers them).**

Every test so far was seeded from what the SUMMARYs describe. A requirement the ROADMAP assigned to this phase but that was silently dropped during execution appears in no SUMMARY, so it would never be tested - UAT could go green while the requirement is unbuilt. Diff the phase's assigned REQ-IDs against what the SUMMARYs claim complete and turn each uncovered REQ-ID into an explicit pending test.

`phase_req_ids` (from init) is the comma-separated list of REQ-IDs the ROADMAP assigns to this phase. **If it is null or empty, skip this step.** Otherwise compute the uncovered set deterministically:

```bash
COVERED=$(for s in "$phase_dir"/*-SUMMARY.md; do
  [ -e "$s" ] || continue
  node "$HOME/.claude/donny/bin/donny-tools.cjs" summary-extract "$s" --fields requirements_completed 2>/dev/null
done | tr ',[]"' '\n' | sed 's/ //g' | grep -E '^[A-Z][A-Z0-9]*-[0-9]+$' | sort -u)

UNCOVERED=$(comm -23 \
  <(echo "$phase_req_ids" | tr ',' '\n' | sed 's/ //g' | grep -E '^[A-Z][A-Z0-9]*-[0-9]+$' | sort -u) \
  <(echo "$COVERED"))
```

For each REQ-ID in `$UNCOVERED`, **prepend** a test (so a dropped requirement is the first thing surfaced, not silently absent):
- name: `"Untested requirement {REQ-ID}"`
- expected: `"{REQ-ID} is assigned to this phase but appears in no SUMMARY. Confirm it is actually built and behaves as REQUIREMENTS.md specifies - if it is missing, this is a dropped requirement, not a passing phase."`

If `$UNCOVERED` is empty, every assigned requirement is represented; add nothing.
</step>
...
```bash
CHECKPOINT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled - /donny-audit-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/donny-audit-phase {phase}` - security review (required before advancing)
- `/donny-plan-phase {next}` - Plan next phase
- `/donny-execute-phase {next}` - Execute next phase
- `/donny-ui-review {phase}` - visual quality audit (if frontend files were modified)
```
...
- `/donny-plan-phase {next}` - Plan next phase
- `/donny-execute-phase {next}` - Execute next phase
- `/donny-audit-phase {phase}` - security review
- `/donny-ui-review {phase}` - visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @$HOME/.claude/donny/workflows/diagnose-issues.md
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► PLANNING FIXES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning planner for gap closure...
```

Spawn donny-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /donny-execute-phase
...
Spawn donny-planner with revision context:
...
3. Abandon (exit, user runs /donny-plan-phase manually)
...
`/clear` then `/donny-execute-phase {phase} --gaps-only`
...
- [ ] If issues: donny-planner creates fix plans (gap_closure mode)
- [ ] If issues: donny-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/donny-execute-phase --gaps-only` when complete
</success_criteria>
