...
<required_reading>
@$HOME/.claude/donny/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init resume)
...
**If `planning_exists` is false:** This is a new project - route to /donny-init
</step>
...
- This is the primary resumption source - structured data from `/donny-pause-work`
...
- **After successful resumption, archive HANDOFF.json** - move it aside rather than deleting, so a session that ends mid-route can still recover the handoff instead of falling back to heuristic scanning:
  ```bash
  mkdir -p .planning/handoffs
  mv .planning/HANDOFF.json ".planning/handoffs/HANDOFF-$(date +%Y%m%d-%H%M%S).json"
  ```
...
📋 [N] pending todos - /donny-capture to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/donny-execute-phase {phase} ${DONNY_WS})
   OR
1. Discuss Phase 3 context (/donny-discuss-phase 3 ${DONNY_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/donny-plan-phase 3 ${DONNY_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** -> Show command for user to run after clearing:
  ...
  **Also available:**
  - `/donny-discuss-phase [N] ${DONNY_WS}` - gather context first
  - `/donny-research-phase [N] ${DONNY_WS}` - investigate unknowns
