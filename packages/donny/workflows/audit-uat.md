...
```bash
AUDIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/donny-verify-work {phase}` - mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/donny-verify-work {phase}`
```
</step>
