...
2. **Run audit first** - `/donny-audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/donny/bin/donny-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `donny-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to donny-tools:**

```bash
ARCHIVE=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```

**Completion gate (engine-enforced, BLOCKING).** Before any archiving, the CLI re-derives
per-phase completion (a phase with plans is complete only when every plan has a SUMMARY,
the same `summaries/plans` formula as `roadmap analyze`). If any phase is incomplete it
archives nothing and returns:
```json
{ "error": "Milestone not complete", "ready": false, "progress_percent": 80, "incomplete": ["05-security-audit"] }
```
On that error, STOP: report the `incomplete` phases and `progress_percent`, and tell the user
to finish/verify them (`/donny-verify-work`, `/donny-execute-phase`). Re-run `milestone complete`
with `--force` ONLY after the user explicitly accepts shipping a milestone with known-incomplete
phases. (This is a distinct axis from the unchecked-requirements gate above: that one is about
REQ-IDs not yet checked off; this one is about phases whose plans have no SUMMARY. Either can
block independently.)

On success the CLI handles:
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/donny-cleanup` later to archive retroactively.

After archival, the AI still handles:
- Reorganizing ROADMAP.md with milestone grouping (requires judgment)
- Full PROJECT.md evolution review (requires understanding)
- Collapsing ROADMAP.md into milestone `<details>` (the file is KEPT) and deleting only REQUIREMENTS.md
- These are NOT fully delegated because they require AI interpretation of content

</step>

<step name="reorganize_roadmap_and_delete_requirements">

After `milestone complete` has archived, reorganize ROADMAP.md with milestone groupings (the file is kept, not deleted), then delete only the requirements original:
...
**Then delete only the requirements original.** Keep `.planning/ROADMAP.md` - it was just collapsed into the milestone `<details>` above and stays as the living roadmap. The full per-phase detail already lives in `milestones/v[X.Y]-ROADMAP.md`. Deleting ROADMAP.md here would destroy the collapse you just wrote and lose the milestone index.
...
**If doesn't exist:** Create from template at `$HOME/.claude/donny/templates/retrospective.md`.
...
**Commit:**
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init execute-phase "1")
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/donny-init)
