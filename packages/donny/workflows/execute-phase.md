...
- **Claude Code:** Uses `Task(subagent_type="donny-executor", ...)` - blocks until complete, returns result
...
These are the valid Donny subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list - do not fall back to 'general-purpose' or other built-in types:

- donny-executor - Executes plan tasks, commits, creates SUMMARY.md
- donny-verifier - Verifies phase completion, checks quality gates
...
- donny-debugger - Diagnoses and fixes issues
- donny-codebase-mapper - Maps project structure and dependencies
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
Check if the current runtime is Copilot by testing for the `@donny-executor` agent pattern
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `$HOME/.claude/donny/workflows/execute-plan.md` **inline**
...
- Maintains Donny's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning Donny
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```

Parse JSON for: `phase`, `plans[]` (each with `id`, `wave`, `autonomous`, `objective`, `files_modified`, `task_count`, `has_summary`), `waves` (map of wave number -> plan IDs), `incomplete`, `has_checkpoints`.

**Authoritative wave source:** each PLAN.md's frontmatter `wave:` field is the source of truth, not the index. The `phase-plan-index` wave value has a known mis-reporting quirk (plans in different waves can both surface as wave 1), which would serialize plans that should run in parallel or vice versa. Read the `wave:` frontmatter directly from each plan file and use that for grouping; if it disagrees with the index, trust the frontmatter. Build the wave map from these values.

**Filtering:** Skip plans where `has_summary: true`. If `--gaps-only`: also skip non-gap_closure plans. If `WAVE_FILTER` is set: also skip plans whose (frontmatter) `wave` does not equal `WAVE_FILTER`.
...
**Action ledger:** record the phase-execution boundary in the per-phase JSONL ledger (fail-safe, never blocks):
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" ledger append "${PHASE_NUMBER}" phase_start --detail "begin execution" >/dev/null 2>&1 || true
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For donny-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @$HOME/.claude/donny/workflows/execute-plan.md
       @$HOME/.claude/donny/templates/summary.md
       @$HOME/.claude/donny/references/checkpoints.md
       @$HOME/.claude/donny/references/tdd.md
       </execution_context>
...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
7. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug - not a Donny or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 5 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS -> treat as **successful**. If spot-checks FAIL -> treat as real failure below.
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/donny/bin/donny-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
**Debugger escalation (checked first, NEVER auto-advanced):** If the checkpoint details include an `Escalate to: donny-debugger` line, the executor exhausted its fix attempts (or node-repair returned ESCALATE) on a failing task. Do NOT spawn a continuation executor and do NOT auto-select an option. Spawn the debugger for a root-cause investigation:
```bash
DEBUGGER_MODEL=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" resolve-model donny-debugger --pick model 2>/dev/null || echo "")
```
```
Task(
  subagent_type="donny-debugger",
  description="Root-cause failed task in phase {phase_number}",
  model="{DEBUGGER_MODEL}",
  prompt="A task failed after the executor's fix attempts. Investigate the root cause (goal: find_root_cause_only; apply a minimal fix only if the cause is unambiguous and low-risk). Paste from the checkpoint: the task name, expected vs actual, the failing command and its output, and the attempts already tried. Return the root cause and, if you fixed it, exactly what changed."
)
```
Record the escalation in the action ledger (fail-safe):
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" ledger append "${PHASE_NUMBER}" escalation --detail "executor exhausted, routed to donny-debugger" >/dev/null 2>&1 || true
```

Handle the debugger return:
- **Root cause found and fixed** -> spawn a continuation executor (continuation-prompt.md) to re-verify the task and finish the plan.
- **Diagnosed only (a human decision is required)** -> present the root cause and the debugger's options to the user; in a fully autonomous chain, stop with the root cause as the blocker. Do NOT proceed past the failing task.
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/donny-execute-phase {phase} ${DONNY_WS}                # Continue remaining waves
/donny-execute-phase {phase} --wave {next} ${DONNY_WS}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${DONNY_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but DONNY_SKIP_SCHEMA_CHECK=true - bypassing gate.
...
2. Skip schema check (DONNY_SKIP_SCHEMA_CHECK=true) - bypass this gate
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/donny-plan-phase {phase} --gaps ${DONNY_WS}` |
...
Commit the file:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` - they will appear in `/donny-progress` and `/donny-audit-uat`.

"approved" -> continue | Report issues -> gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/donny-verify-work` on it.
...
`/donny-plan-phase {X} --gaps ${DONNY_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` - full report
Also: `/donny-verify-work {X} ${DONNY_WS}` - manual testing first
```

Gap closure cycle: `/donny-plan-phase {X} --gaps ${DONNY_WS}` reads VERIFICATION.md -> creates gap plans with `gap_closure: true` -> user runs `/donny-execute-phase {X} --gaps-only ${DONNY_WS}` -> verifier re-runs.
</step>
...
```bash
COMPLETION=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/donny-progress` and `/donny-audit-uat`.
```

```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `$HOME/.claude/donny/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/donny-transition` command. Never suggest it. The transition workflow is internal only.**
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` -> Claude Code bug, not Donny. Spot-check (SUMMARY exists, commits present) -> if pass, treat as success
...
<resumption>
Re-run `/donny-execute-phase {phase}` -> discover_plans finds completed SUMMARYs -> skips them -> resumes from first incomplete plan -> continues wave execution.
