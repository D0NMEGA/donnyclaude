...
Seeds auto-surface during /donny-init when trigger conditions match the
...
- Auto-present at the right time via the donny-init milestone scan
</purpose>
...
```bash
# Next seed number = highest existing SEED number + 1.
# Use the max, NOT the count: counting collides if any seed was ever deleted.
HIGHEST=$( (ls .planning/seeds/SEED-*.md 2>/dev/null || true) \
  | sed -E 's#.*/SEED-0*([0-9]+).*#\1#' | sort -n | tail -1 )
NEXT=$(( ${HIGHEST:-0} + 1 ))
...
status: dormant
planted: {ISO date}
planted_during: {current milestone/phase from STATE.md, or "pre-roadmap" if STATE.md is absent}
...
This seed should be presented during `/donny-init` when the milestone
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs: plant seed - {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /donny-init
