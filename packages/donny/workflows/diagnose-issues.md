...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-debugger 2>/dev/null)
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
