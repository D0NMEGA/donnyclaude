...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /donny-env-new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /donny-env-new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /donny-init    # Initialize Donny in the workspace
```
...
**Offer to initialize Donny (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize Donny"
- question: "Would you like to initialize a Donny project in the new workspace?"
- options:
  - "Yes - run /donny-init" -> tell user to `cd $TARGET_PATH` first, then run `/donny-init`
