...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel donny-codebase-mapper agents.

Use Task tool with `subagent_type="donny-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `donny-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="donny-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `donny-codebase-mapper` agent. Include actual file paths formatted with backticks.

Continue to verify_output.
</step>

<step name="verify_output">
Verify every expected document was created and is substantive (deterministic completeness gate - a partial map must not be committed as complete):

```bash
EXPECTED="STACK.md INTEGRATIONS.md ARCHITECTURE.md STRUCTURE.md CONVENTIONS.md TESTING.md CONCERNS.md"
MISSING=""; THIN=""
for f in $EXPECTED; do
  p=".planning/codebase/$f"
  if [ ! -f "$p" ]; then MISSING="$MISSING $f"
  elif [ "$(grep -c . "$p")" -lt 20 ]; then THIN="$THIN $f"; fi
done
echo "missing:${MISSING:- none} | thin:${THIN:- none}"
```

- `MISSING` non-empty: a mapper agent failed to produce that doc. Map each missing file to its focus area (STACK/INTEGRATIONS = tech, ARCHITECTURE/STRUCTURE = arch, CONVENTIONS/TESTING = quality, CONCERNS = concerns) and re-run that mapper. Do NOT commit a partial map.
- `THIN` non-empty (a file under 20 non-blank lines): that mapper produced a stub - re-run it.
- Proceed only when both are `none`.

Continue to scan_for_secrets.
</step>

<step name="scan_for_secrets">
**CRITICAL SECURITY CHECK (engine-enforced, BLOCKING):** Scan the generated docs for leaked secrets before committing. The patterns are versioned and tested in the engine, and a finding never echoes the credential in full:

```bash
SCAN=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" security scan-secrets .planning/codebase/*.md)
```

Returns `{ clean, files_scanned, missing, findings: [{ file, kind, line, preview }] }` and exits non-zero when not clean. (The commit step below re-runs this and will not commit unless it passes, so the gate holds even if this step is skipped.)
...
{For each finding: file:line - kind (preview)}

This would expose credentials if committed.

**Action required:**
1. Open each flagged file:line and remove or redact the secret.
2. Re-run the scan until `clean` is true.
```

Do NOT continue to commit_codebase_map while `clean` is false. Only the user may waive a finding they confirm is a documentation example / false positive.
...
Commit the codebase map - but only if the secret scan passes. The `&&` makes the commit structurally conditional on a clean scan (exit 0), so a leaked key cannot reach git history even if the scan_for_secrets step above was skipped:

```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" security scan-secrets .planning/codebase/*.md >/dev/null \
  && node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md \
  || echo "Commit blocked: secrets detected in .planning/codebase/*.md. Resolve the findings and retry."
```
...
**Also available:**
- Re-run mapping: `/donny-map-codebase`
...
- If Task tool available: 4 parallel donny-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- Completeness gate passed - all 7 codebase documents exist and are substantive (>=20 non-blank lines); no partial map committed
- Secret scan passed - `security scan-secrets` clean, and the commit is structurally gated on it
- Clear completion summary with line counts
- User offered clear next steps in Donny style
</success_criteria>
