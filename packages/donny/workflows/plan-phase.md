<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates donny-phase-researcher, donny-planner, and donny-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
- donny-phase-researcher - Researches technical approaches for a phase
- donny-planner - Creates detailed plans from phase scope
- donny-plan-checker - Reviews plan quality before execution
</available_agent_types>
...
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-phase-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-plan-checker 2>/dev/null)
CONTEXT_WINDOW=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
/donny-review --phase {N}

Then re-run /donny-plan-phase {N} --reviews
```
Exit workflow.
...
5. Commit:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call - AskUserQuestion
  ...
  ```
  Run this command first, then re-run /donny-plan-phase {X} ${DONNY_WS}:

  /donny-discuss-phase {X} ${DONNY_WS}
  ```
...
### Browser research notice (one-time, per user)

Research agents can open a real Chrome through the Playwright MCP to read JS-gated or login-walled pages. Surface this once per user before the first research run. Reference: @~/.claude/donny/references/browser-research.md

```bash
[ -f "$HOME/.donny/.browser-notice-ack" ] && echo "browser_notice=acked" || echo "browser_notice=new"
```

**Show the notice only when `browser_research_enabled` is true AND `browser_notice=new`.** Otherwise skip to the banner. Skip entirely under `--auto` (non-interactive).

If `TEXT_MODE` is true, present as a plain-text numbered list; otherwise use AskUserQuestion:
```
AskUserQuestion([
  {
    question: "donny's research agents can open a real Chrome through the Playwright MCP to read live docs and inspect UIs that plain HTTP cannot reach. They try HTTP and Context7 first and open the browser only when a page needs it, so a Chrome window may appear depending on how your Playwright MCP is configured. Keep browser research on, or use HTTP-only? You can change this anytime with /donny-settings.",
    header: "Browser research",
    multiSelect: false,
    options: [
      { label: "Keep on (Recommended)", description: "Research agents may use the Playwright MCP browser when a page needs it. Configure the MCP headless to avoid a visible window." },
      { label: "Disable (HTTP-only)", description: "Research stays HTTP and Context7 only and never opens a browser." }
    ]
  }
])
```

Always record the acknowledgement so the notice appears once per user:
```bash
mkdir -p "$HOME/.donny" && touch "$HOME/.donny/.browser-notice-ack"
```

**If the user chose "Disable (HTTP-only)":** persist it for this project and future ones, and treat `browser_research_enabled` as false for the rest of this run:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" config-set workflow.browser_research false
node -e 'const f=require("os").homedir()+"/.donny/defaults.json",fs=require("fs");let d={};try{d=JSON.parse(fs.readFileSync(f,"utf8"))}catch(e){}d.workflow=Object.assign({},d.workflow,{browser_research:false});fs.writeFileSync(f,JSON.stringify(d,null,2))'
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn donny-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**Browser research:** {If `browser_research_enabled` is true: "Enabled. You may use the Playwright MCP (mcp__playwright__*) for JS-gated or login-walled pages when HTTP and Context7 are not enough. Prefer HTTP, Context7, and WebFetch first; open the browser only when a page truly needs it." Otherwise: "Disabled. Use only HTTP (WebFetch/WebSearch), Context7, and the docs tools. Do not use the Playwright MCP or open a browser."}
</additional_context>
...
Task(
  ...
  subagent_type="donny-phase-researcher",
...
**If found:**
1. Read template: `$HOME/.claude/donny/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Each PLAN.md must include a <threat_model> block.
Block on: {SECURITY_BLOCK} severity threats.
Opt out: config-set workflow.security_enforcement false
```
...
```bash
UI_PHASE_CFG=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="donny-ui-phase", args="${PHASE} --auto ${DONNY_WS}")
```
After `donny-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
1. Generate UI-SPEC first - Run /donny-ui-phase {N} then re-run /donny-plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" -> Display: "Run `/donny-ui-phase {N} ${DONNY_WS}` then re-run `/donny-plan-phase {N} ${DONNY_WS}`". Exit workflow.
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
Clear any stale replan-escalation checkpoint from a prior run before this attempt begins. A fresh plan-phase run invalidates the previous verdict: if it converges, no checkpoint remains and the autonomous detector passes; if it re-hits the cap, step 12 writes a fresh one. (Mirrors discuss-phase clearing its DISCUSS-CHECKPOINT.)

```bash
rm -f "${PHASE_DIR}/${padded_phase}-REPLAN-CHECKPOINT.json"
...
```bash
_donny_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_donny_field "$INIT" state_path)
ROADMAP_PATH=$(_donny_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_donny_field "$INIT" requirements_path)
RESEARCH_PATH=$(_donny_field "$INIT" research_path)
VERIFICATION_PATH=$(_donny_field "$INIT" verification_path)
UAT_PATH=$(_donny_field "$INIT" uat_path)
CONTEXT_PATH=$(_donny_field "$INIT" context_path)
REVIEWS_PATH=$(_donny_field "$INIT" reviews_path)
```
...
1. Re-run: `/donny-plan-phase {PHASE} --research ${DONNY_WS}`
2. Disable Nyquist with the exact command:
   `node "$HOME/.claude/donny/bin/donny-tools.cjs" config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn donny-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /donny-execute-phase. Plans need:
...
**If "Split":** Use `/donny-phase-add` to create the sub-phases, then replan each.
...
## 10. Spawn donny-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  ...
  subagent_type="donny-plan-checker",
  model="{checker_model}",
...
## 12. Revision Loop (bounded by `max_replan_iterations`, default 2)

Track `iteration_count` (starts at 1 after initial plan + check). The cap is `max_replan_iterations` (parsed from init; default 2). This bounds the plan<->check loop so an autonomous run cannot replan forever.

**If iteration_count < `max_replan_iterations`:**

Display: `Sending back to planner for revision... (iteration {N}/{max_replan_iterations})`
...
**If iteration_count >= `max_replan_iterations` (cap reached):**

The plan and checker did not converge. Escalate to a decision checkpoint and STOP. This is **unconditional** -- the safe default for both interactive and autonomous runs. Do NOT force-proceed, do NOT execute checker-rejected plans, and do NOT block on an interactive menu an autonomous run cannot answer.

```bash
cat > "${PHASE_DIR}/${padded_phase}-REPLAN-CHECKPOINT.json" <<JSON
{"phase": "{phase_number}", "kind": "replan_escalation", "iterations": {iteration_count}, "max_replan_iterations": {max_replan_iterations}, "unresolved_issues": {structured_issues_from_checker}}
JSON
node "$HOME/.claude/donny/bin/donny-tools.cjs" ledger append "${PHASE}" escalation --detail "replan cap reached, decision needed" >/dev/null 2>&1 || true
```

Then emit `## PLANNING ESCALATED` with the unresolved issues and the checkpoint path, and STOP (do not continue to step 13). The autonomous orchestrator detects this checkpoint (autonomous.md step 3b) and routes to `handle_blocker` -- a decision pause, not a failure. To resume: rerun `/donny-plan-phase {X} --reviews` after addressing the issues, or raise `workflow.max_replan_iterations` and rerun.

> Why unconditional: the autonomous path does not signal `--auto` to plan-phase, so gating the safe stop on an auto-mode flag would silently expose a force-proceed foot-gun on exactly the run type that most needs the stop. A fresh plan-phase run clears any prior checkpoint at the start of step 6, so this verdict never goes stale.
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
2. **Sync chain flag with intent** - if user invoked manually (no `--auto` and no `--chain`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]] && [[ ! "$ARGUMENTS" =~ --chain ]]; then
     node "$HOME/.claude/donny/bin/donny-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
Skill(skill="donny-execute-phase", args="${PHASE} --auto --no-transition ${DONNY_WS}")
```
...
- **PHASE COMPLETE** -> Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   DONNY ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /donny-discuss-phase ${NEXT_PHASE} --auto ${DONNY_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** -> Display result, stop chain:
  ...
  /donny-execute-phase ${PHASE} ${DONNY_WS}
  ```
...
/donny-execute-phase {X} ${DONNY_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md - review plans
- /donny-plan-phase {X} --research - re-research first
- /donny-review --phase {X} --all - peer review plans with external AIs
- /donny-plan-phase {X} --reviews - replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/donny-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/donny-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] donny-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] donny-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] donny-plan-checker spawned with CONTEXT.md
