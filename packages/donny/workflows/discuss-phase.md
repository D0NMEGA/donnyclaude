<purpose>
Extract implementation decisions that downstream agents need. Analyze the phase, surface Claude's key assumptions for correction, identify gray areas, let the user choose what to discuss, then deep-dive each selected area until satisfied.
...
1. **donny-phase-researcher** - Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" -> researcher investigates card component patterns
   - "Infinite scroll decided" -> researcher looks into virtualization libraries

2. **donny-planner** - Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/donny-discuss-phase --text`)
- Per-project: `donny-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/donny-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" agent-skills donny-discuss-researcher 2>/dev/null)
```
...
Use /donny-progress ${DONNY_WS} to see available phases.
```
Exit workflow.
...
- Read and execute @$HOME/.claude/donny/workflows/discuss-phase-power.md end-to-end
...
- options:
  - "Continue and replan after" - Capture context, then run /donny-plan-phase {X} ${DONNY_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/donny/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" resolve-model donny-discuss-researcher --raw)
   ```
...
Before asking the user anything, surface what Claude is *assuming* about this phase so wrong assumptions get corrected early - far cheaper than discovering them during planning or execution. This step always runs: it is the analysis half of discussion (what Claude believes), complementing the intake half (what the user knows).

Draw on `analyze_phase` (domain, prior decisions, gray areas) and `codebase_context`. State assumptions across five areas, each tagged with a confidence level - `Fairly confident` (clear from roadmap/prior phases), `Assuming` (reasonable inference), or `Unclear` (could go several ways):

1. **Technical approach** - libraries, frameworks, and patterns Claude would reach for. Ground these in scouted code ("reuse the existing Card component"), not generic guesses.
2. **Implementation order** - what gets built first, second, third, and why.
3. **Scope boundaries** - what's in scope, what's out, and which edges are ambiguous.
4. **Risk areas** - where Claude expects complexity, and the parts worth watching.
5. **Dependencies** - what this assumes from prior phases, external dependencies, and what future phases consume from it.

Present them in one scannable block:
```
## Assumptions for Phase [X]: [Name]

### Technical approach
[assumptions, each with a confidence tag]
...
### Dependencies
From prior phases: [...]    External: [...]    Feeds into: [...]

---
**What do you think - anything wrong or missing?**
```

**If `--auto`:** Do not block. Log `[auto] Surfaced assumptions for Phase [X]; no corrections gathered (auto mode).` and continue to present_gray_areas - the assumptions still inform the auto-selected gray areas.

**Otherwise:** Wait for the user's response, then fold it back into the analysis:
- A corrected assumption either pre-answers a gray area (drop it from the upcoming menu) or sharpens one (reword it). Update the gray-area set before present_gray_areas.
- If the user names a doc, spec, or ADR while correcting ("the auth flow lives in docs/adr-007"), add it to the `<canonical_refs>` accumulator immediately (Source 4).
- If the user confirms with no changes, note "Assumptions validated" and continue.

Corrections captured here are decisions: they belong in CONTEXT.md's Implementation Decisions when write_context runs, the same as anything surfaced during discuss_areas.
</step>
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$HOME/.claude/agents/donny-discuss-researcher.md for your role and instructions.
...
This is a structured checkpoint, not the final CONTEXT.md - the `write_context` step still produces the canonical output. But if the session dies, the next `/donny-discuss-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
`/donny-plan-phase ${PHASE} ${DONNY_WS}`

---

**Also available:**
- `/donny-discuss-phase ${PHASE} --chain ${DONNY_WS}` - re-run with auto plan+execute after
- `/donny-plan-phase ${PHASE} --skip-research ${DONNY_WS}` - plan without research
- `/donny-ui-phase ${PHASE} ${DONNY_WS}` - generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="donny-plan-phase", args="${PHASE} --auto ${DONNY_WS}")
```
...
- **PHASE COMPLETE** -> Full chain succeeded. Display:
  ...
  Next: /donny-discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${DONNY_WS}
  ```
- **PLANNING COMPLETE** -> Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /donny-execute-phase ${PHASE} ${DONNY_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** -> Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /donny-plan-phase ${PHASE} ${DONNY_WS}
  ```
- **GAPS FOUND** -> Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /donny-plan-phase ${PHASE} --gaps ${DONNY_WS}
  ```
...
**Full step-by-step instructions:** @$HOME/.claude/donny/workflows/discuss-phase-power.md
...
- Key assumptions surfaced across the five areas and offered for correction before the gray-area menu (always; in `--auto`, logged without blocking)
