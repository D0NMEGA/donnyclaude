<purpose>
Remove a Donny workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/donny-env-list` to show available workspaces, then ask:
