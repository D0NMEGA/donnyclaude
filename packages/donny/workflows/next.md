<purpose>
Detect current project state and automatically advance to the next logical Donny workflow step.
...
```bash
# Get state snapshot
node "$HOME/.claude/donny/bin/donny-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No Donny project detected. Run `/donny-init` to get started.
```
Exit.
</step>
...
-> Next action: `/donny-discuss-phase <first-phase>`
...
▶ **Next step:** `/donny-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation - the whole point of `/donny-next` is zero-friction advancement.
</step>
