...
There is no `/donny-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/donny-transition`.

**Valid user commands for phase progression:**
- `/donny-discuss-phase {N}` - discuss a phase before planning
- `/donny-plan-phase {N}` - plan a phase
- `/donny-execute-phase {N}` - execute a phase
- `/donny-progress` - see roadmap progress
...
These will carry forward as debt. Review: `/donny-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to donny-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `donny-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `donny-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `DONNY_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$DONNY_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$DONNY_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/donny-plan-phase [X+1] --auto ${DONNY_WS}")
...
Exit skill and invoke SlashCommand("/donny-discuss-phase [X+1] --auto ${DONNY_WS}")
...
`/donny-discuss-phase [X+1] ${DONNY_WS}` - gather context and clarify approach

---

**Also available:**
- `/donny-plan-phase [X+1] ${DONNY_WS}` - skip discussion, plan directly
- `/donny-research-phase [X+1] ${DONNY_WS}` - investigate unknowns
...
`/donny-plan-phase [X+1] ${DONNY_WS}`

---

**Also available:**
- `/donny-discuss-phase [X+1] ${DONNY_WS}` - revisit context
- `/donny-research-phase [X+1] ${DONNY_WS}` - investigate unknowns
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/donny-workstreams complete {current_ws_name} ${DONNY_WS}`

See overall milestone progress:

`/donny-workstreams progress ${DONNY_WS}`
...
Do NOT suggest `/donny-complete-milestone` or `/donny-init`.
...
Exit skill and invoke SlashCommand("/donny-complete-milestone {version} ${DONNY_WS}")
...
`/donny-complete-milestone {version} ${DONNY_WS}`
