...
**If `roadmap_exists` is false:** Error - "No ROADMAP.md found. Run `/donny-init` first."
**If `state_exists` is false:** Error - "No STATE.md found. Run `/donny-init` first."
...
```bash
ROADMAP=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap analyze)
```

Parse the JSON `phases` array.

**Filter to actionable phases:** Keep only phases where `actionable === true`. The engine computes this field (`roadmap analyze`): a phase is actionable when it is not yet complete (incomplete on disk OR its ROADMAP box unchecked - the original rule) AND it has not been explicitly deferred via a `*-DEFERRED.md` marker. Filtering on `actionable` excludes phases a prior run deferred, so they are never re-prompted.
...
```bash
DETAIL=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="donny:discuss-phase", args="${PHASE_NUM}")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="donny-ui-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"donny:plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="donny-plan-phase", args="${PHASE_NUM}")
```

**Replan-escalation check (bounded auto-loop):** plan-phase escalates instead of force-proceeding when the plan<->check loop hits `max_replan_iterations` with unresolved issues. If it left a checkpoint, the plans did NOT pass the checker; do not execute them.
```bash
ls "${PHASE_DIR}"/*-REPLAN-CHECKPOINT.json 2>/dev/null
```
If a `*-REPLAN-CHECKPOINT.json` exists -> go to handle_blocker: "Plan phase ${PHASE_NUM} escalated after max_replan_iterations revision rounds; planner and checker could not converge. Review the checkpoint, then resume with /donny-plan-phase ${PHASE_NUM} --reviews, or raise workflow.max_replan_iterations and retry."
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"donny:execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="donny-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
On **"Continue without validation"**: Mark the phase deferred (it is not complete - human sign-off is still outstanding - so this records the reason without re-prompting it):
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" phase defer ${PHASE_NUM} --reason "human verification deferred"
```
Display `Phase ${PHASE_NUM} ⏭ Human validation deferred` and proceed to iterate step.
...
```
Skill(skill="donny-plan-phase", args="${PHASE_NUM} --gaps")
```
...
On "Continue anyway": Defer the phase (gaps remain, so it is not complete), then proceed to iterate step:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" phase defer ${PHASE_NUM} --reason "gaps remain after closure attempt"
```
On "Stop autonomous mode": Go to handle_blocker.

This limits gap closure to 1 automatic retry to prevent infinite loops.

On **"Continue without fixing"**: Defer the phase so the loop advances without recording it complete, then proceed to iterate step:
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" phase defer ${PHASE_NUM} --reason "gaps deferred by user"
```
Display `Phase ${PHASE_NUM} ⏭ Gaps deferred` and proceed to iterate step.
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="donny-ui-review", args="${PHASE_NUM}")
```

Display the review result summary (score from UI-REVIEW.md if produced). Continue to the mutation gate (3d.6) regardless of score - UI review is advisory, not blocking.

**If `UI_SPEC_FILE` is empty OR `UI_REVIEW_CFG` is `false`:** Skip silently to the mutation gate (3d.6).

**3d.6. Mutation gate (soft)**

> Run after 3d.5 UI Review, before the iterate step. This workflow is the autonomous (--auto) path, so the gate runs here.
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `donny-discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `donny-discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /donny:autonomous --from ${next_incomplete_phase}
```
...
Re-filter actionable phases using the same logic as discover_phases:
- Keep phases where `actionable === true` (excludes complete phases and ones deferred this run, so a deferred phase is not re-selected - this is what breaks the re-prompt loop)
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} - Done
 Mode: Single phase (--only)

 Lifecycle skipped - run /donny-autonomous without --only
...
```
Skill(skill="donny-audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped - audit gaps remain. Run /donny-audit-milestone to review, then /donny-complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped - tech debt to address. Run /donny-audit-milestone to review details."

**5b. Complete Milestone**

**Guard - orphan phase directories.** `donny-complete-milestone` counts accomplishments from `.planning/phases/`. Directories left over from a prior, already-archived milestone make it miscount, prepend a garbled MILESTONES entry, and can target ROADMAP.md for deletion. Before completing, check that every `.planning/phases/<dir>` maps to a phase number in the current ROADMAP.md:

```bash
ROADMAP_PHASES=$(grep -oE 'Phase[[:space:]]+[0-9]+' .planning/ROADMAP.md 2>/dev/null | grep -oE '[0-9]+' | sort -u)
ORPHANS=""
for d in .planning/phases/*/; do
  [ -d "$d" ] || continue
  n=$(basename "$d" | grep -oE '^[0-9]+' | sed 's/^0*//')
  [ -z "$n" ] && continue
  echo "$ROADMAP_PHASES" | grep -qx "$n" || ORPHANS="${ORPHANS} $(basename "$d")"
done
```

**If `ORPHANS` is non-empty:** do NOT call complete-milestone yet. Show the orphan directories and ask via AskUserQuestion:
- **Archive orphans first** - run `Skill(skill="donny-cleanup")` to archive the stale directories, then re-run this guard.
- **Continue anyway** - proceed knowing the accomplishment count may be off (not recommended).
- **Stop** - go to handle_blocker so the user can sort the directories out manually.

Only once `.planning/phases/` is free of orphans (or the user explicitly chose to continue) call:

```
Skill(skill="donny-complete-milestone", args="${milestone_version}")
```
...
**On "Skip this phase":** Mark the phase deferred so the next iterate does not re-select it (without this the skipped phase stays the lowest actionable phase and the loop re-runs it forever):
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" phase defer ${PHASE_NUM} --reason "skipped by user"
```
Log `Phase {N} ⏭ {Name} - Skipped by user` and proceed to iterate.

**On "Stop autonomous mode":** Display progress summary:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 DONNY ► AUTONOMOUS ▸ STOPPED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /donny-autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="donny-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via donny:discuss-phase (asks questions, waits for user)
