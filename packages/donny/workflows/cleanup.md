...
Parse `$ARGUMENTS` for the `--dry-run` flag:
...
When `DRY_RUN=true`, the workflow prints the archival summary and stops without prompting, moving, or committing anything (useful in CI or scripted contexts).
...
**If `DRY_RUN=true`:** print the summary above and stop here - do not prompt, archive, or commit.
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "chore: archive phase directories from completed milestones" --files .planning/milestones/ .planning/phases/
```
