...
Post-mortem investigation for failed or stuck Donny workflows. Analyzes git history,
...
Scope the artifact table to the CURRENT milestone only. Phase dirs left over from completed
milestones (not yet archived by `/donny-cleanup`) lack SUMMARY/VERIFICATION by design, and
including them produces false-positive "missing artifact" anomalies.

```bash
# Phase numbers in the current ROADMAP (= current milestone)
ROADMAP_PHASES=$( (grep -oE 'Phase [0-9]+' .planning/ROADMAP.md 2>/dev/null || true) | grep -oE '[0-9]+' | sort -u )
ls -d .planning/phases/*/
```

For each phase directory whose leading number IS in `ROADMAP_PHASES`, check which artifacts exist:
...
Phase dirs whose number is NOT in `ROADMAP_PHASES` belong to a prior/completed milestone: list
them once under "Excluded (prior milestone)" and do NOT flag their missing artifacts.

Track: which current-milestone phases have complete artifact sets vs gaps.
...
Check for orphaned worktrees (from crashed agents). **Exclude expected worktrees** before treating
extras as a crash signal: the cco-dream runner creates and reconciles `dream/run-*` worktrees as
part of normal operation, so a `dream/run-*` entry is NOT evidence of a crash. Only worktrees beyond
the main one that do NOT match `dream/run-*` count toward the crash/interruption signal in Step 3.

### 2f. Action ledger (execution event trail)

For each current-milestone phase, read the append-only action ledger - the structured record of
`phase_start` / `verification` / `escalation` events the executor writes during a run. It is the
closest thing donny has to a flight recorder, and the primary evidence for WHEN and WHY a run
stalled or escalated.

```bash
for n in $( (grep -oE 'Phase [0-9]+' .planning/ROADMAP.md 2>/dev/null || true) | grep -oE '[0-9]+' | sort -u ); do
  node "$HOME/.claude/donny/bin/donny-tools.cjs" ledger tail "$n" 50 --raw 2>/dev/null
done
```

Track per phase: any `escalation` events (executor exhausted, node-repair, debugger handoff); the
last event (did the phase reach `verification`, or stop after `phase_start`?); and any gap between
the final ledger event and the last commit. A phase with a `phase_start` but no `verification`, or a
trailing `escalation`, points straight at where the run broke - feed these into Step 3.
...
- `git worktree list` shows worktrees beyond the main one, excluding expected `dream/run-*` runner worktrees
...
3. {Recovery command if applicable - e.g., `/donny-resume-work`, `/donny-execute-phase N`}

---

*Report generated by `/donny-forensics`. All paths redacted for portability.*
```
...
```bash
donny-tools.cjs state record-session \
