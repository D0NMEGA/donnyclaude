...
<required_reading>
@$HOME/.claude/donny/references/verification-patterns.md
@$HOME/.claude/donny/templates/verification-report.md
</required_reading>
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase "${phase_number}"
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap analyze
```
...
Use donny-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use donny-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" verify artifacts "$plan")
...
Use donny-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" verify key-links "$plan")
...
See $HOME/.claude/donny/templates/verification-report.md for complete template.
</step>
