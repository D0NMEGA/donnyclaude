...
4. If nothing found: error "No milestone found. Run /donny-init or /donny-init first."
...
```bash
donny-tools.cjs commit "docs(v${VERSION}): generate milestone summary for onboarding" \
...
If the user is done:
- Suggest next steps: `/donny-init`, `/donny-progress`, or sharing the summary with the team
