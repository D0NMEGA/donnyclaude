<purpose>
Create all phases necessary to close gaps identified by `/donny-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases - no manual `/donny-phase-add` per gap.
</purpose>
...
```
No audit gaps found. Run `/donny-audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/donny-execute-phase {N}` - if plans already exist
...
`/donny-audit-milestone` - re-audit to verify gaps closed
`/donny-complete-milestone {version}` - archive when audit passes
```
...
- [ ] User knows to run `/donny-plan-phase` next
</success_criteria>
