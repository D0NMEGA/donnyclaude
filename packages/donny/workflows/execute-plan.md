...
@$HOME/.claude/donny/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via donny-tools config-get if needed
```
...
2. Per segment:
   - Subagent route: spawn donny-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" phases list --type summaries --raw
...
See `$HOME/.claude/donny/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node $HOME/.claude/donny/bin/donny-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See $HOME/.claude/donny/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/donny/bin/donny-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/donny/workflows/node-repair.md` with:
...
If `NODE_REPAIR` is `false` OR repair returns ESCALATE: do NOT silently defer a red task. Escalate to the debugger so the orchestrator can spawn a root-cause investigation. Return a `## CHECKPOINT REACHED` message with `**Type:** decision`, an `**Escalate to:** donny-debugger` line, the flags `symptoms_prefilled: true` and `goal: find_root_cause_only`, and attach: Task [X] name, expected vs actual, the failing command and its output, and the repair attempts already tried. First (recommended) option: escalate to donny-debugger. Other options: Skip (mark incomplete) | Stop. This escalation is returned even in auto mode (a failing task is never silently shipped); record it in SUMMARY.md and set the SUMMARY `status:` to `PARTIAL`.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$HOME/.claude/donny/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$HOME/.claude/donny/templates/summary.md`.
...
Update STATE.md using donny-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/donny/bin/donny-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/donny/bin/donny-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/donny/bin/donny-tools.cjs" state record-metric \
...
node "$HOME/.claude/donny/bin/donny-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/donny/bin/donny-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using donny-tools:
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/donny/bin/donny-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/donny-execute-phase {phase}` + `/donny-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/donny-plan-phase {Z+1}` + `/donny-verify-work {Z}` + `/donny-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/donny-complete-milestone` + `/donny-verify-work` + `/donny-phase-add` |
