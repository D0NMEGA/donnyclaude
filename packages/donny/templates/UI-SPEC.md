...
> Visual and interaction contract for frontend phases. Generated by donny-ui-researcher, verified by donny-ui-checker.
...
> Add a row per third-party registry ONLY after vetting it. The Safety Gate cell must
> hold timestamped evidence (`view passed - no flags - {date}` or
> `developer-approved after view - {date}`), never an intent note like "view required"
> (the checker BLOCKs dimension 6 on intent-only gates). If there are no third-party
> registries (shadcn official only, or no shadcn), omit the extra row entirely.
