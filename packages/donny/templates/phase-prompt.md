...
> **Note:** Planning methodology is in `agents/donny-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @$HOME/.claude/donny/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/donny-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `$HOME/.claude/donny/references/tdd.md` for TDD plan structure.
...
See `$HOME/.claude/donny/templates/user-setup.md` for full schema and examples
...
See `$HOME/.claude/donny/workflows/verify-phase.md` for verification logic.
