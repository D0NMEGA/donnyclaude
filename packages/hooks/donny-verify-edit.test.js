#!/usr/bin/env node
// Tests for donny-verify-edit.js. No test framework dependency, runs under
// plain node so it can execute in hook-land without installing devDeps.
//
// Usage: node packages/hooks/donny-verify-edit.test.js
...
const HOOK = path.join(__dirname, 'donny-verify-edit.js');
const mod = require('./donny-verify-edit.js');
...
function testDetectNpmLint() {
  const tmp = mktmp('donny-verify-a');
  fs.writeFileSync(
...
function testFixturePassthrough() {
  const tmp = mktmp('donny-verify-d');
  ...
  const projTmp = mktmp('donny-verify-d2');
  fs.writeFileSync(
...
// Run all.
process.stdout.write('donny-verify-edit.test.js\n');
