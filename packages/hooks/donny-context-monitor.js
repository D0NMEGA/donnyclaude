...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect if Donny is active (has .planning/STATE.md in working directory)
    const isDonnyActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));

    // Build advisory warning message (never use imperative commands that
    // override user preferences — see #884)
    let message;
    if (isCritical) {
      message = isDonnyActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          'Context is nearly exhausted. Do NOT start new complex work or write handoff files — ' +
          'Donny state is already tracked in STATE.md. Inform the user so they can run ' +
          '/donny-pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
