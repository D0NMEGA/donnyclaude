#!/usr/bin/env node
// Unit tests for donny-session-start hook logic.
// Run with: node packages/hooks/donny-session-start.test.js
...
const {
  ...
  parseStdin,
  PACKAGE_MANAGERS,
} = require('./donny-session-start.js');
...
function mkTmp(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'donny-session-start-' + prefix + '-'));
}
