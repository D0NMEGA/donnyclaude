---
name: donny-debug
description: "Systematic root-cause debugging by the scientific method, with session state persisted across context resets. Use when investigating a bug, error, test failure, or unexpected behavior, or resuming a prior debug session; pass --diagnose to find the root cause without applying a fix."
...
**Orchestrator role:** Gather symptoms, spawn donny-debugger agent, handle checkpoints, spawn continuations.
...
```bash
debugger_model=$(node "$HOME/.claude/donny/bin/donny-tools.cjs" resolve-model donny-debugger --raw)
```
...
- Offer options:
  ...
  - "Plan fix" - suggest `/donny-plan-phase --gaps`
  - "Manual fix" - done

**If `## DEBUG COMPLETE` (find_and_fix mode):**
- Display root cause and fix summary
- Offer options:
  - "Plan fix" - suggest `/donny-plan-phase --gaps` if further work needed
  - "Done" - mark resolved
...
- [ ] donny-debugger spawned with context
